...
sub build_digest {
    ...
    open my $fh, '<', $self->path or 
        confess 'Cannot open file ' . $self->path;
    binmode $fh;

    my $data;
    my $alg = $self->settings->checksum;
    load_class $alg;
    my $digest = $alg eq 'Digest::xxHash' ? $alg->new(0) : $alg->new; 
...
sub find_duplicates {
    ...
    if (scalar (@files) == 1 and not $files[0]->has_digest) {
        if (File::Compare::compare($self->path, $files[0]->path, $self->settings->block_size ) == 0) {
...
sub remove {
    ...
    if ($self->settings->remove) {
        unlink $self->path or
            confess "Unable to remove ". $self->path;
    }
}   
